...
try {
    ...
    $query = "SELECT file_path FROM location_photos WHERE photo_id = :photo_id";
    ...
    if ($photo) {
        ...
        if ($deleteStmt->execute()) {
            ...
            // config.php의 BASE_PATH (프로젝트 루트)를 기준으로 파일 경로 생성
...
// 9. 작업 완료 후, 메시지와 함께 장소 수정(edit.php) 페이지로 복귀
if ($error) {
    redirect('/admin/locations/edit.php?id=' . $location_id . '&message=' . urlencode($error) . '&type=error');
} else {
    redirect('/admin/locations/edit.php?id=' . $location_id . '&message=' . urlencode($success));
}
