...
async function uploadToGoogleDrive(mode) {
    ...
    /* Cloude code */
    const region = document.getElementById('region-select').value || '지역미선택';
    const category = document.getElementById('category-select').value || '카테고리미선택';
    const place = document.getElementById('place-select').value || '장소미선택';
    const treeSpecies = document.getElementById('tree-select').value || '나무종류미선택';
    
    // ⭐ 폴더명: 지역_카테고리_장소_나무종류_날짜시간
    const now = new Date();
    ...
    const folderName = `${region}_${category}_${place}_${treeSpecies}_${dateTime}`;
...
async function createSpreadsheet(imagesToUpload, folderId) {
 ...
  
        const headerRow = [
            '사진번호', '지역', '카테고리', '장소', '수종', '높이(m)', '둘레(cm)', '상태','파일명','크기(KB)','위도', '경도', '전체주소', '시/도', '시/군/구', '동/읍/면', '카메라', '촬영날짜'
          ];
...
if (treeHeightInput) {
    ...
    // 모든 입력 이벤트 처리
    treeHeightInput.addEventListener('input', function(e) {
    ...
    // 한글 입력 시작 차단
    treeHeightInput.addEventListener('compositionstart', function(e) {
        console.log('⚠️ 한글/중국어/일본어 입력 차단됨');
    });
    
    // 한글 입력 종료 시 즉시 제거
    treeHeightInput.addEventListener('compositionend', function(e) {
    ...
    // 붙여넣기
    treeHeightInput.addEventListener('paste', function(e) {
        e.preventDefault();
        const pasteData = (e.clipboardData || window.clipboardData).getData('text');
        const cleaned = pasteData.replace(/[^0-9.]/g, '');
        
        const parts = cleaned.split('.');
        const finalData = parts.length > 1 ? parts[0] + '.' + parts.slice(1).join('') : cleaned;
        
        document.execCommand('insertText', false, finalData);
    });
    
    // 드래그 앤 드롭 차단
    treeHeightInput.addEventListener('drop', function(e) {
        e.preventDefault();
        return false;
    });
    
    console.log('✅ 나무 높이: 한글 포함 모든 문자 차단');
}

// 나무 둘레 (정수만 허용)
const treeThicknessInput = document.getElementById('treeThickness');
if (treeThicknessInput) {
    let lastValidValue = '';
    
    // 모든 입력 이벤트 처리
    treeThicknessInput.addEventListener('input', function(e) {
        let value = this.value;
    ...
    // 한글 입력 시작 차단
    treeThicknessInput.addEventListener('compositionstart', function(e) {
        console.log('⚠️ 한글/중국어/일본어 입력 차단됨');
    });
    
    // 한글 입력 종료 시 즉시 제거
    treeThicknessInput.addEventListener('compositionend', function(e) {
        this.value = this.value.replace(/[^0-9]/g, '');
    });
    
    // 붙여넣기
    treeThicknessInput.addEventListener('paste', function(e) {
        e.preventDefault();
        const pasteData = (e.clipboardData || window.clipboardData).getData('text');
        const cleaned = pasteData.replace(/[^0-9]/g, '');
    ...
    // 드래그 앤 드롭 차단
    treeThicknessInput.addEventListener('drop', function(e) {
        e.preventDefault();
        return false;
    });
    
    console.log('✅ 나무 둘레: 한글 포함 모든 문자 차단');
}

// ====================================
// 🚨 추가: Placeholder를 보호 (입력 힌트 유지)
// ====================================

// HTML placeholder가 지워지지 않도록 보호
if (treeHeightInput && !treeHeightInput.placeholder) {
    treeHeightInput.placeholder = '예: 5.5';
}
if (treeThicknessInput && !treeThicknessInput.placeholder) {
    treeThicknessInput.placeholder = '예: 120';
}

console.log('🔒 한글 입력 완전 차단 시스템 활성화 (IME 대응)');
...
async function executeMerge() {
 ...
        // 2. 각 스프레드시트에서 데이터 가져와서 통합
        let allRows = [[ '사진번호', '지역', '카테고리', '장소','수종', '높이(m)', '둘레(cm)', '상태', '파일명', '크기(KB)', '위도', '경도', '전체주소', '시/도', '시/군/구', '동/읍/면', '카메라', '촬영날짜', '출처폴더' ]];
        
...
               `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(sheetTitle)}!A17:R`,
               { headers: { 'Authorization': `Bearer ${accessToken}` } }
            );
 ...
                console.warn(`시트(${sheetId})에서 'A17:R' 범위 데이터를 읽는 데 실패했습니다. 건너뜁니다.`);
                continue; // 다음 루프로 넘어감
            }
            
 ...
                
                rows.forEach(row => {
                    const newRow = row.slice(0, 18); // A~R (18개)
                    while (newRow.length < 18) newRow.push(''); // 혹시 모를 빈 셀 채우기
 ...
        
        // 5. 최종 성공 메시지
        // ✅ [수정] '확인' 버튼이 포함된 새 함수를 호출합니다.
        const successMsg = createMergeSuccessMessage(
            spreadsheetIds.length,
            allRows.length - 1, // 헤더 제외
            mergedSpreadsheetId
        );
        
