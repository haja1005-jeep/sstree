...
        const tree = document.getElementById('tree-select').value;
        ...
        // 2. 폴더 이름의 기본이 될 제목입니다.
        const folderParts = ['스마트 트리맵 현장사진']; 
        
        // 3. 값이 선택된 필터만 이름에 추가합니다.
        if (region) folderParts.push(region);
        if (category) folderParts.push(category);
        if (place) folderParts.push(place);
        if (tree) folderParts.push(tree);

        // 4. 배열을 ' - ' 문자로 연결하여 최종 폴더 이름을 만듭니다.
        let folderName = folderParts.join(' - ');

        // 5. (안전장치) 만약 아무 필터도 선택하지 않았다면,
        //    폴더 이름이 겹치지 않도록 오늘 날짜를 뒤에 붙입니다.
        if (folderParts.length === 1) {
            folderName += ` ${new Date().toLocaleDateString('ko-KR')}`;
        }
