...
const GOOGLE_CLIENT_ID = '204515737075-4i39uitvagjk1pvqthrjdcgfuaptmjga.apps.googleusercontent.com';  
const GOOGLE_API_KEY = 'AIzaSyBMC8rNXYHNFXMXAMteN011xfivCGbaTgo';
...
async function uploadToGoogleDrive(mode) {
    ...
    try {
        ...
        const spreadsheetId = await createSpreadsheet(imagesToUpload, folderId);
        ...
        successMsg.innerHTML = `
            <span>✅</span>
            <span>${imagesToUpload.length}개의 이미지, 상세 정보 텍스트, 스프레드시트가 성공적으로 업로드되었습니다!</span>
        `;
        progressContainer.appendChild(successMsg);

        setTimeout(() => {
            const message = `업로드가 완료되었습니다!\n\n` +
                `📁 폴더 보기\n` +
                `📊 스프레드시트 보기\n\n` +
                `어떤 것을 열까요?`;
