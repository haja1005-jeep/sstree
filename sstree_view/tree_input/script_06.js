// Google Drive API Configuration (config.js에서 로드)  - 클로드 코드 생성 시프레드시트 통합 성공
...
async function createSpreadsheet(imagesToUpload, folderId) {
    try {
        ...
        if (!createResponse.ok) {
            ...
            throw new Error(`스프레드시트 생성 실패: ${errorData.error?.message || '알 수 없는 오류'}`);
        }
        ...
        if (!summaryResponse.ok) {
            ...
            throw new Error(`요약 정보 입력 실패: ${errorData.error?.message || '알 수 없는 오류'}`);
        }
        
        console.log('✅ 요약 정보 입력 완료');
        ...
        if (!dataResponse.ok) {
            ...
            throw new Error(`사진 데이터 입력 실패: ${errorData.error?.message || '알 수 없는 오류'}`);
        }
        
        console.log('✅ 사진 데이터 입력 완료');
        ...
        throw new Error(`스프레드시트 생성 중 오류: ${error.message}`);
    }
}
...
async function loadSpreadsheetData(spreadsheetId) {
    try {
        ...
        // ⭐️ 먼저 스프레드시트의 시트 정보를 가져와서 실제 시트 이름 확인
        ...
        console.log('📋 시트 이름:', firstSheetTitle);
...
async function executeMerge() {
    try {
        ...
        const loadingMsg = document.createElement('div');
        loadingMsg.className = 'progress-item';
        loadingMsg.innerHTML = `<span>📊 ${spreadsheetIds.length}개의 스프레드시트 통합 중...</span>`;
        progressContainer.appendChild(loadingMsg);
        ...
        // ⭐️ 통합 스프레드시트의 첫 번째 시트 이름 가져오기
        const mergedSheetTitle = newSheet.sheets[0].properties.title;
        
        console.log('✅ 통합 스프레드시트 생성:', mergedSpreadsheetId, '시트 이름:', mergedSheetTitle);
        
        // 각 스프레드시트에서 데이터 가져와서 통합
        ...
        for (let i = 0; i < spreadsheetIds.length; i++) {
            ...
            try {
                // ⭐️ 먼저 각 스프레드시트의 시트 정보를 가져와서 실제 시트 이름 확인
                const sheetInfoResponse = await fetch(
                ...
                console.log(`📋 스프레드시트 ${i + 1} 시트 이름:`, firstSheetTitle);
                ...
                if (data.values && data.values.length > 1) {
                    ...
                    rows.forEach(row => {
                        row.push(`스프레드시트 ${i + 1}`); // 출처 표시
                        allRows.push(row);
                    });
                    console.log(`✅ ${i + 1}/${spreadsheetIds.length} 데이터 수집 완료 (${rows.length}행)`);
                } else {
                    console.log(`⚠️ ${i + 1}/${spreadsheetIds.length} 데이터 없음`);
                }
            } catch (error) {
                console.error(`❌ 스프레드시트 ${i + 1} 데이터 수집 실패:`, error);
                // 한 개 실패해도 계속 진행
            }
        }
        
        // ⭐️ 통합된 데이터 입력 (동적 시트 이름 사용)
        await fetch(
        ...
        console.log('✅ 통합 데이터 입력 완료:', allRows.length, '행');
